        ...
        if len(testAstr.Subparty) == len(testBstr.Subparty) {
          ...
          for bigI := 0; bigI < len(testAstr.Subparty) ; bigI++ {
            ...
            for i := 0; i < len(testA.Parameters) ; i++ {
              ...
              for x:= 0 ; x < 64 ; x++ {
                var masker uint64 = uint64(math.Pow(2, float64(x)))
                fmt.Printf("Masker: %d\n", masker)
                testNumber := testA.Parameters[i] & masker
                fmt.Printf("testNumber after & with A : %d\n", testNumber)
                testNumber = testNumber + (testB.Parameters[i] & masker)
                fmt.Printf("testNumber after & with B : %d\n", testNumber)
                if (testNumber > masker)  {
                   finalNumber = finalNumber + masker
