...
func (t *SimpleAsset) Invoke(ctx contractapi.TransactionContextInterface, A [] string, C string) error {
	// Extract the function and args from the transaction proposal
        ...
        testAstr := []Metadata{}
        testAstr = make([]Metadata, 10, 10)
        testAstr = nil

        //STORE METADATA
        for i := 0; i < len(A) ; i++ {
          Astrbytes, err := ctx.GetStub().GetState(A[i])
          fmt.Printf("Astrbytes: %b\n\n", Astrbytes )
          if err != nil {
                return fmt.Errorf("Failed to get state")
          }
          tempTestAstr := new(Metadata)
          _ = json.Unmarshal(Astrbytes, tempTestAstr)
          testAstr = append(testAstr, *tempTestAstr)
        }

        //GET NUMBER OF WORKER NODES
        numberOfNodes := len(testAstr)
        fmt.Printf("Number of nodes: %d\n\n", numberOfNodes)

        if len(testAstr[0].Subparty) == len(testAstr[1].Subparty) {
          ...
          for batchNumber := 0; batchNumber < len(testAstr[0].Subparty) ; batchNumber++ {
            var batch []Payload
            batch = make([]Payload, 10, 10)
            batch = nil

            for nodeNumber := 0; nodeNumber < numberOfNodes; nodeNumber++ {
              Avalbytes, err := ctx.GetStub().GetState(testAstr[nodeNumber].Subparty[batchNumber])
              if err != nil {
                return fmt.Errorf("Failed to get state")
              }
            ...
            for i := 0; i < len(batch[0].Parameters) ; i++ {
              ...
              for x:= 0 ; x < 64 ; x++ {
                ...
                //TRACE LOG
                //fmt.Printf("Masker: %d\n", mask)                
                var testNumber uint64 = 0

                //For each worker node
                for nodeNumber := 0; nodeNumber < numberOfNodes; nodeNumber++ {
                  testNumber = testNumber +  batch[nodeNumber].Parameters[i] & mask
                  //TRACE LOG
                  //fmt.Printf("testNumber after & with node %d : %d\n", nodeNumber, testNumber)
                }
                // END of for each worker node || testNumber is addition of all the bits
                if (testNumber > (mask*uint64(numberOfNodes/2)))  {
                   finalNumber = finalNumber + mask
                   //TRACE LOG
                   //fmt.Printf("Final Number : %d\n", finalNumber)
                }
              }
              //END of For each Bit of the 64 bit Parameter || finalNumber is the majority
              fmt.Printf("Final Number : %d\n", finalNumber)
              Cval = append(Cval, finalNumber)
              fmt.Printf("length of Cval[%d] : %d",i,Cval[i])
              fmt.Printf("Subparty: %d",batchNumber)
            }
            //END of For each 64 bit Parameter in a Batch || finalNumber is loaded into Cval

            Cstr := []string {C + strconv.Itoa(batchNumber)}
            Ctest := Payload{
                Parameters: Cval,
            }
...
// either a 'get' or a 'set' on the asset created by Init function. The Set
// method may create a new asset by specifying a new key-value pair.
func (t *SimpleAsset) Invoketest(ctx contractapi.TransactionContextInterface, A, B, C string) error {
	// Extract the function and args from the transaction proposal
        var err error
        var Cval []uint64
        Cval = make([]uint64, 4687500, 4687500)
        ...
        if len(testAstr.Subparty) == len(testBstr.Subparty) {
          ...
          for bigI := 0; bigI < len(testAstr.Subparty) ; bigI++ {
            ...
            for i := 0; i < len(testA.Parameters) ; i++ {
              fmt.Printf("OnesCount64(%064b) = %d\n", testA.Parameters[i], bits.OnesCount64(testA.Parameters[i]))
              fmt.Printf("OnesCount64(%064b) = %d\n", testB.Parameters[i], bits.OnesCount64(testB.Parameters[i]))
              var finalNumber uint64 = 0
              for x:= 0 ; x < 64 ; x++ {
                var mask uint64 = uint64(math.Pow(2, float64(x)))
                fmt.Printf("Masker: %d\n", mask)
                testNumber := testA.Parameters[i] & mask
                fmt.Printf("testNumber after & with A : %d\n", testNumber)
                testNumber = testNumber + (testB.Parameters[i] & mask)
                fmt.Printf("testNumber after & with B : %d\n", testNumber)
                if (testNumber > mask)  {
                   finalNumber = finalNumber + mask
                   fmt.Printf("Final Number : %d\n", finalNumber)
                }
