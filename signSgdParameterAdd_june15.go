...
func (t *SimpleAsset) Invoke(ctx contractapi.TransactionContextInterface, A [] string, C string, beg int, end int) error {
	// Extract the function and args from the transaction proposal
        ...
        if len(testAstr[0].Subparty) == len(testAstr[1].Subparty) {
          ...
          tempCstr = nil
          if (beg != 0) {
            Cstrbytes, err := ctx.GetStub().GetState(C)
            if err != nil {
                return fmt.Errorf("Failed to get state")
            }
            tempTestCstr := new(Metadata) 
            _ = json.Unmarshal(Cstrbytes, tempTestCstr)
            tempCstr = append(tempCstr, tempTestCstr.Subparty...)
          }
